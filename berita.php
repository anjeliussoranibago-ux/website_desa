...
$search = $_GET['search'] ?? '';
$query = "SELECT * FROM berita_informasi";
$params = [];
...
<head>
    ...
    <title>Kelola Berita - Desa Hilifalago</title>
...
<body>
                    ...
                    <div class="pt-0 pb-0 px-3 d-flex flex-column flex-grow-1" style="overflow: hidden;">
                        <div class="card shadow-sm glass-card border-0 mb-0 mt-0 flex-grow-1 d-flex flex-column" style="max-height: 100%; overflow: hidden; border-bottom-left-radius: 0 !important; border-bottom-right-radius: 0 !important;">
                            <div class="card-header bg-transparent border-bottom pt-3 px-4 pb-3 d-flex justify-content-between align-items-center">
                                <h5 class="fw-bold text-secondary mb-0"><i class="fa-solid fa-newspaper me-2"></i>Daftar Berita & Pengumuman</h5>
                                <a href="tambah_berita.php" class="btn btn-primary btn-sm shadow-sm"><i class="fa-solid fa-plus me-1"></i> Buat Berita Baru</a>
                            </div>
                                    ...
                                    <table class="table table-hover table-bordered align-middle mb-0">
                                        ...
                                        <tbody>
                                            <?php if (count($data_berita) > 0): ?>
                                                <?php $no = 1; foreach ($data_berita as $row): ?>
                                                <tr>
                                                    <td class="text-center"><?= $no++ ?></td>
                                                    <td class="text-center">
                                                        <?php if (!empty($row['gambar_cover']) && file_exists('berita_img/' . $row['gambar_cover'])): ?>
                                                            <img src="berita_img/<?= htmlspecialchars($row['gambar_cover']) ?>?t=<?= time() ?>" class="rounded shadow-sm" style="width: 60px; height: 40px; object-fit: cover;">
                                                        <?php else: ?>
                                                            <div class="rounded shadow-sm bg-light text-muted d-inline-flex align-items-center justify-content-center" style="width: 60px; height: 40px; font-size: 0.7rem;">No Img</div>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td class="fw-bold text-primary text-wrap" style="max-width: 250px;"><?= htmlspecialchars($row['judul']) ?></td>
                                                    <td class="text-center"><?= date('d M Y, H:i', strtotime($row['tanggal_publikasi'])) ?></td>
                                                    <td class="text-center">
                                                        <span class="badge <?= $row['status'] == 'Published' ? 'bg-success' : 'bg-secondary' ?>"><?= htmlspecialchars($row['status']) ?></span>
                                                    </td>
                                                    <td class="text-center">
                                                        <button onclick="konfirmasiHapus(<?= $row['id_berita'] ?>)" class="btn btn-sm btn-danger border-0" title="Hapus Berita"><i class="fa-solid fa-trash"></i></button>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="6" class="text-center text-muted py-4">Belum ada berita yang dipublikasikan.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
    ...
    <script>
        function konfirmasiHapus(id) { Swal.fire({ title: 'Hapus Berita?', text: "Berita yang dihapus tidak dapat dikembalikan!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Ya, Hapus!', cancelButtonText: 'Batal' }).then((result) => { if (result.isConfirmed) { window.location.href = 'hapus_berita.php?id=' + id; } }) }
